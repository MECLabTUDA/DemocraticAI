...
class ClientUpdate:
    def __init__(self, model, client_name, training_round=None, meta=None, vectorized=False, normalized_vectors=True, encrypted=False):
        ...
        if encrypted:
            self.state_dict      = model.meta["encrypted_weights"]
            self.state_dict_oneD = model.meta["encrypted_weights"]['all'] 
        elif vectorized:
            self.state_dict_oneD = self.get_state_dict_oneD(self.state_dict)
        else:
