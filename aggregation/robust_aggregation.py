...
from aggregation.ASMR import ASMR

class Aggregator(BaseFedAvg):
    def __init__(
        self,
        *args,
        num_clients: int = 3,
        num_rounds:  int = 5,
        start_round: int = 0,
        apply_homomorphic_encryption: bool = False,
        algo: str = 'FedAvg',
        **kwargs,
    ):
    ...
    @staticmethod
    def _get_algo(algo, apply_homomorphic_encryption):
        if algo == 'FedAvg':
            return WeightedAggregationHelper()
        elif algo == 'ASMR':
            return ASMR(apply_homomorphic_encryption=apply_homomorphic_encryption)
    ...
    @staticmethod
    def aggregate_fn(results: List[FLModel], algo: str) -> FLModel:
        if not results:
            raise ValueError("received empty results for aggregation.")

        aggr_helper = Aggregator._get_algo(algo, False)
        aggr_metrics_helper = Aggregator._get_algo(algo, False)
        all_metrics = True
        for _result in results:
            aggr_helper.add(_result)
    ...
    def aggregate(self, results: List[FLModel], aggregate_fn=None) -> FLModel:
        ...
        try:
            aggr_result = aggregate_fn(results, self.algo)
