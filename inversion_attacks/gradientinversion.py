...
from .utils.distance import cossim, l2, l2_normalized
...
class GradientInversion_Attack(BaseAttacker):
    ...
    def _setup_distancefunc(self, distancename):
        ...
        elif distancename == "l2_normalized":
            self.distancefunc = l2_normalized
    ...
    def attack(
        ...
        for i in range(1, self.num_iteration+1):
            ...
            if torch.sum(torch.isnan(distance)).item():
                ...
                print("stop because the culculated distance is Nan")
                break
    ...
    def group_attack(self, received_gradients, batch_size=1):
        ...
        dm, ds = self.mean_std
        num_of_not_improve_round = torch.zeros(self.group_num)
        ...
        for i in tqdm.trange(1, self.num_iteration + 1):
            ...
            for worker_id in range(self.group_num):
                ...
                if best_distance[worker_id] > distance:
                    ...
                    num_of_not_improve_round[worker_id] = 0
                else:
                    num_of_not_improve_round[worker_id] += 1
            ...
            # END: for worker_id in range(self.group_num)

            if torch.all(num_of_not_improve_round > self.early_stopping):
                print(
                    f"iter={i}: loss did not improve in the last {self.early_stopping} rounds for any worker."
                )
                break
        # END: for i in tqdm.trange(1, self.num_iteration + 1)
