...
class SimpleImageNet(torch.utils.data.Dataset):
    """A simple ImageNet dataset that returns images and labels."""

    def __init__(self, root, split='train', transform=None):
        self.root = "/local/scratch/imagenet/train"
        ...
        for i, c in enumerate(self.classes):
            ...
            for img_name in os.listdir(class_dir):
                ...
                self.samples.append((img_path, c, i))
                break  # only take one sample per class
    ...
    def __getitem__(self, idx):
        img_path, label_str, label_idx = self.samples[idx]
...
def _build_imagenet(data_path, augmentations=True, normalize=True):
    ...
    trainset = SimpleImageNet(root=None, split=None, transform=transforms.ToTensor())
