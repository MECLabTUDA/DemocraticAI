...
from network.BasicNCA2D import BasicNCA2DFast
import random
import math
...
MED_NCA_DICT = {
    ...
    'model.train.patch_sizes': [None, None], # [[80, 80, 6], None]
    'model.train.loss_weighted_patching': False,

    'model.eval.patch_wise': False,

    'model.levels.res_and_steps': [[[200, 192], 20], [[50, 48], 40]],
    'model.levels.separate_models': True,
    ...
    'performance.inplace_operations': False,

}



class MedNCA(torch.nn.Module):
    r"""Implementation of M3D-NCA
    """
    def __init__(self, config: dict=MED_NCA_DICT):
        r"""Init function
            #Args:
        ...
        levels_res_and_steps = config['model.levels.res_and_steps']
        separate_models = config['model.levels.separate_models']
        ...
        self.levels_res = [tuple(r_s[0]) for r_s in levels_res_and_steps]
        self.inference_steps = [r_s[1] for r_s in levels_res_and_steps]
        ...
        if separate_models:
            if isinstance(kernel_size, list):
                assert len(kernel_size) == len(levels_res_and_steps), "kernel_size must have same length as levels_res_and_steps"
            else:
                kernel_size = [kernel_size] * len(levels_res_and_steps)
        else:
        ...
        assert backbone_class in [BasicNCA2DFast], f"backbone_class must be either BasicNCA2D, got {backbone_class}"

        if separate_models:
            if config["model.vitca"]:
                self.backbone_ncas = []
                for l in range(len(levels_res_and_steps)):
        ...
        for i in range(len(self.levels_res)-1):
            t = []
            for c in range(2):
                t.append(self.levels_res[i][c]//self.levels_res[i+1][c])
    ...
    def forward(self, x: torch.Tensor, y: torch.Tensor = None, batch_duplication=1):
        ...
        if self.training:
            assert x.shape[1:3] == self.levels_res[0], f"Expected shape {self.levels_res[0]}, got shape {x.shape[1:3]}"
    ...
    def downscale(self, x: torch.Tensor, level: int):
        ...
        out = F.interpolate(x, size=self.levels_res[level])
        out.names = ('B', 'C', 'H', 'W')
        x.names = ('B', 'C', 'H', 'W')
        return out
    ...
    def forward_train(self, x: torch.Tensor, y: torch.Tensor, batch_duplication=1):
        ...
        if self.patch_sizes[-1] is not None:
            ...
            for b in range(x.shape[0]):
                if self.loss_weighted_patching:
                    ...
                    h_start = self.my_rand_int(0, self.levels_res[-1][0]-self.patch_sizes[-1][0])
                    w_start = self.my_rand_int(0, self.levels_res[-1][1]-self.patch_sizes[-1][1])
            ...
            x_new = torch.zeros(x.shape[0], *self.levels_res[-1], self.channel_n,
                                dtype=torch.float, device=self.device)
            current_patch = np.array([[[0,0], [*self.levels_res[-1]]]] * x.shape[0])
            x = self.downscale(x, -1)
            x = self.align_tensor_to(x, "BHWC")
            self.remove_names(x)
        ...
        for level in range(len(self.levels_res)-1, -1, -1):
    ...
    def forward_eval(self, x: torch.Tensor):
        ...
        if x.shape[1:3] != self.levels_res[0]:
            temp_levels_res = self.levels_res
            new_levels_res = [list(x.shape[1:3])]
            for i in range(1, len(self.levels_res)):
                downsample_factor = np.array(self.levels_res[i-1]) / np.array(self.levels_res[i])
                new_levels_res.append([math.ceil(new_levels_res[i-1][0] / downsample_factor[0]), 
                                        math.ceil(new_levels_res[i-1][1] / downsample_factor[1])])
            self.levels_res = new_levels_res
            print("running inference on different resolution, this is the new resolution:", self.levels_res)


        pred, _ = self.forward_train(x, x)

        if x.shape[1:3] != tuple(self.levels_res[0]):
            self.levels_res = temp_levels_res
    ...
    def compute_probabilities_matrix(self, loss: torch.Tensor, level: int) -> torch.Tensor:
        ...
        loss = F.interpolate(loss, size=self.levels_res[level])
...
def create_med_nca_us(device):
    config = MED_NCA_DICT
    config['experiment.device'] = device
    config['model.levels.res_and_steps'] = [[[64, 64], 20], [[32, 32], 40]]
    return MedNCA(config=config)

def create_med_nca_xray(device):
    config = MED_NCA_DICT
    config['experiment.device'] = device
    config['model.levels.res_and_steps'] = [[[64, 64], 20], [[32, 32], 40]]
    return MedNCA(config=config)

class MedNCAHAM(MedNCA):
    def __init__(self, device):
        config = MED_NCA_DICT
        config['experiment.device'] = device
        config['model.levels.res_and_steps'] = [[[192, 256], 20], [[48, 64], 40]]
        config['model.input_channels'] = 3

        super().__init__(config)

def create_med_nca_ham(device):
    config = MED_NCA_DICT
    config['experiment.device'] = device
    config['model.levels.res_and_steps'] = [[[192, 256], 20], [[48, 64], 40]]
    config['model.input_channels'] = 3
    return MedNCAHAM(device=device)
